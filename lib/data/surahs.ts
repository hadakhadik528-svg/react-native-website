...
export const SURAHS: Surah[] = NAMES.map(([name, ayahs], i) => ({
  n: i + 1,
  name,
  ayahs,
  type: MEDINAN.has(i + 1) ? 'مدنية' : 'مكية',
}));

export const BASMALA = 'بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ';

export function surahAudioUrl(server: string, n: number): string {
  return `${server}${String(n).padStart(3, '0')}.mp3`;
}
